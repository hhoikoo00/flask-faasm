"""
Adapted from: https://github.com/python/pyperformance/blob/main/pyperformance/data-files/benchmarks/bm_scimark/run_benchmark.py
"""
    ...
    for p in range(cycles):
        for y in range(1, G.height - 1):
            for x in range(1, G.width - 1):
                G[x, y] = (
                    omega * 0.25 * (G[x, y - 1] + G[x, y + 1] + G[x - 1, y] + G[x + 1, y]) + (1.0 - omega) * G[x, y]
                )


def bench_SOR(n, cycles, Array):
    range_it = range(1)
...
def bench_SparseMatMult(N, nz):
    ...
    return SparseCompRow_matmult(N, y, val, row, col, x, 1)
...
def bench_MonteCarlo(Num_samples):
    range_it = range(1)
    t0 = time.perf_counter()
...
def bench_scimark():
    for _, (bench_fn, *args) in BENCHMARKS.items():
        bench_fn(*args)
